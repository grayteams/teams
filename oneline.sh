...
command -v curl >/dev/null 2>&1 || { echo "[-] butuh curl (apt install curl)"; exit 1; }
...
if [ ! -f "$BIN" ]; then
    ...
    curl -fsSL -o "$BIN" "${BASE_URL}/${BIN}" || { echo "[-] Gagal unduh ${BIN}"; exit 1; }
fi
if [ ! -f "$BIN.sha256" ]; then
    echo "[+] Unduh checksum ${BIN}.sha256..."
    curl -fsSL -o "$BIN.sha256" "${BASE_URL}/${BIN}.sha256" || { echo "[-] Gagal unduh checksum"; exit 1; }
fi
