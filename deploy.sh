...
# Kill all Mandarin + watchdog + .cache processes
ps aux | grep -E '系统|核心|缓存|管理|数据库|维护|守护|进程|网络|存储|日志|监控|引擎|调度|备份|同步|服务|安全|认证|配置' | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null
...
while :; do
    ...
    [ -f "${WD}/${BIN}" ] && { cd "$WD"; ./"${BIN}" -c config.json >/dev/null 2>&1; }
    sleep 0.1
done
WDOGEOF
